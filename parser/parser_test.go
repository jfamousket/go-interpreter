...
func TestIfExpression(t *testing.T) {
	...
	if len(exp.Consequence.Statements) != 1 {
		t.Fatalf("consequence is not %d statements. got=%T", 1, len(exp.Consequence.Statements))
	}
...
func TestCallExpression(t *testing.T) {
	...
	if len(callexpression.Arguments) != 3 {
		t.Fatalf("call expression parameters wrong. got=%d\n", len(callexpression.Arguments))
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("progam.Statements does not contain %d statements. got=%d\n", 1, len(program.Statements))
		}
...
func testBooleanLiteral(t *testing.T, bl ast.Expression, value bool) bool {
	b, ok := bl.(*ast.Boolean)
	if !ok {
		t.Errorf("t not *ast.Boolean. got=%T", b)
		return false
	}
